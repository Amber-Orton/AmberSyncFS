...
void upload_file(const std::string& relative_path) {
    ...
    if (create_event_checked("UF", relative_path, mod_time) < 0) {
...
void delete_file(const std::string& relative_path) {
    ...
    if (create_event_checked("DF", relative_path, current_time) < 0) {
...
void upload_directory(const std::string& relative_path) {
    ...
    if (create_event_checked("UD", relative_path, mod_time) < 0) {
...
void delete_directory(const std::string& relative_path) {
    ...
    if (create_event_checked("DD", relative_path, current_time) < 0) {
